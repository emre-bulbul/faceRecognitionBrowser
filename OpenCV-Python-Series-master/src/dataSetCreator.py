...
def insertOrUpdate(id, name, password, mail):
    ...
    if(isRecordExist == 1):
        cmd = "UPDATE People SET Name=' " + \
            str(name)+" ', Password='"+str(password) + \
            "', Email='"+str(mail)+"' WHERE ID="+str(id)

    else:
        cmd = "INSERT INTO People(Id,Name,Password,Email) Values(" + \
            str(id)+",' "+str(name)+" ,'"+str(password)+",'"+str(mail)+" )"
